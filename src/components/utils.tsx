...
export function updateTextDisplays(language: string, level: number) {
  ...
  if (restartButton != null) { restartButton.innerHTML = layout.restart };
  // language button
  let languageButton = document.getElementById("language");
  if (languageButton != null) { languageButton.innerHTML = layout.language.toUpperCase() };
  // level button
  let levelButton = document.getElementById("level");
  if (levelButton != null) { levelButton.innerHTML = layout.level + " : " + layout.levels[level].toUpperCase() };
  // about button
  let aboutButton = document.getElementById("about");
  if (aboutButton != null) { aboutButton.innerHTML = layout.about };
  return (null)
}     
